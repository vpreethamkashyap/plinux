...
//#define MIN_PEAK_AMP	30 // Prevents detections of peaks smaller than 150 uV.
...
/*	Initialize all buffers to 0 on the first call.	*/
	if( init )
		...
		Peak(0,1) ;
		RRMean = RRCount = RRLastVal = RRFlag = 0;
		}
		...
		if(newPeak > 0)
			...
			// if(!BLSCheck(DDBuffer, DDPtr, &maxder))
				...
				if(newPeak > det_thresh)
					...
					qmean = mean(qrsbuf,8) ;
					det_thresh = thresh(qmean,nmean) ;
			// Skip RR intervals less than 360ms
					rrtemp = count - WINDOW_WIDTH;
					if (rrtemp >= MS360)
					{
					memmove(&rrbuf[1], rrbuf, MEMMOVELEN) ;
					rrbuf[0] = count - WINDOW_WIDTH + rrprev ;
					printf("Case 1 - %d \n",rrbuf[0]);
					if (RRFlag)
					{
					RRMean += rrbuf[0];
			...
			//		RRMean = rrmean;
				...
				// Store the peak for possible search back.
					...
					{
					//printf("Noise  Threshold - %d Peak - %d  RR - %d \n",det_thresh,newPeak,(count - WINDOW_WIDTH));
				...
				//else
				//	printf("BLSCheck Fail New Peak - %d RR - %d\n",newPeak,(count - WINDOW_WIDTH));
			}
