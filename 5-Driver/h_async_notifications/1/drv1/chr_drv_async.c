...
#define CHAR_DEV_NAME 		"async_chr_drv"
...
static int char_dev_write(struct file *file,const char *buf,size_t lbuf,loff_t *ppos)
{
	...
	if( async_struct ) 
	{
		...
		kill_fasync(&async_struct, SIGIO, POLL_IN); // poll_in for read and poll_out for write
	}
	*ppos += nbytes;
	return nbytes;
}
...
/*********************************************************************************
 * Function:			char_dev_fasync
 * Description:		 
 ********************************************************************************/
static int char_dev_fasync(int fd,struct file *filp,int mode)
{
	printk("Calling fasync in driver");
	return fasync_helper(fd, filp, mode, &async_struct); // which returns the PID
}
...
static __init int char_dev_init(void)
{
	...
	device_create (struct_class, NULL, mydev, NULL, "%s", "async_chr_drv");
