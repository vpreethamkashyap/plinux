...
#define CHAR_DEV_NAME 		"kSynch_mutex"
...
static struct class *class_struct;

int count=1;
static char *char_device_buf;

static int RecursiveLock(void);
...
static ssize_t char_dev_read(struct file *file,char *buf,size_t lbuf,loff_t *ppos)
{
  ...
  //  Testing for accidental release 
	...
	{
		printk(KERN_DEBUG "Something went wrong with wait in Read\n");
	...
	*ppos += nbytes;
	printk(KERN_DEBUG "READER has completed its job\n");
	return nbytes;	
}
...
static ssize_t char_dev_write(struct file *file,const char *buf,size_t lbuf,loff_t *ppos)
{
	...
	if(mutex_lock_interruptible(&mylock)) 	// begin of critical region 
	{
		printk(KERN_DEBUG "Something went wrong with wait in Write\n");
		return -ERESTARTSYS;									// something went wrong with wait 
	}
...
static int RecursiveLock(void)
{
	...
	{
		printk(KERN_DEBUG "Something went wrong with wait in RecursiveLock\n");
		return -ERESTARTSYS;		// something went wrong with wait 
	}
...
static __init int char_dev_init(void)
{
	...
	cdev_init(cdev_struct,&char_dev_fops);
	
	ret=cdev_add(cdev_struct,mydev,count);	
	if( ret < 0 ) 
	{
	...
	class_struct = class_create (THIS_MODULE, "VIRTUAL");
  device_create (class_struct, NULL, mydev, NULL, "%s", "kSynch_mutex");
	printk(KERN_INFO"\nDevice Registered: %s\n",CHAR_DEV_NAME);
...
{
	device_destroy (class_struct, mydev);
