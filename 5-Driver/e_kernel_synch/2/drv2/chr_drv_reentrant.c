...
#define CHAR_DEV_NAME 		"kSynch_reentarnt"
...
static ssize_t char_dev_read(struct file *file,char *buf,size_t lbuf,loff_t *ppos)
{
	...
	if(down_interruptible(&mysem))	//begin of critical region		
	{
	...
	up(&mysem); // end of critical region 
	*ppos += nbytes;
	return nbytes;	
}
...
static __init int char_dev_init(void)
{
	...
	device_create (struct_class, NULL, mydev, NULL, "%s", "kSynch_reentarnt");
	printk(KERN_INFO"\nDevice Registered: %s\n",CHAR_DEV_NAME);
